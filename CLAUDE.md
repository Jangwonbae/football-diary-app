...
`ReviewEntity`의 `emotionTags`는 Gson으로 직렬화된 JSON 배열 문자열로 저장되며, `toDomain()`에서 `List<String>`으로 역직렬화된다.
...
수정 요청을 받으면 실제로 파일을 수정하기 전에 변경할 파일명과 전체 수정 내용을 먼저 보여주고, 사용자의 확인을 받은 후에 수정한다.

수정 내용을 보여줄 때는 변경된 줄에 주석을 달아 어디가 바뀌었는지 명확히 표시한다:
- 추가된 줄: 해당 줄 끝에 `// 추가` 주석
- 삭제된 줄: 해당 줄 끝에 `// 삭제` 주석
